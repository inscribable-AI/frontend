...
export function AgentStats({ toolAgents, superAgents, tasks }) {
  ...
  useEffect(() => {
    if ((!toolAgents && !superAgents) || !tasks) return;
    ...
    const totalToolAgents = toolAgents?.length || 0;
    const totalSuperAgents = superAgents?.length || 0;
    ...
    allAgents.forEach(agent => {
      agentTaskMap[agent.id] = {
        ...
        completedTasks: 0,
        hasRecentTask: false,
        isToolAgent: agent.id.startsWith('TA_')
      };
    });
  ...
  return (
    <div className="bg-white dark:bg-gray-800 shadow-lg rounded-xl overflow-hidden">
        ...
        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-gray-700 dark:to-gray-800 rounded-lg p-4 shadow-sm">
          ...
          <div className="mt-3 flex items-center text-sm">
            <span className="text-gray-600 dark:text-gray-300">
              <span className="font-medium text-blue-500">{stats.totalToolAgents}</span> Tool, 
              <span className="font-medium text-purple-500 ml-1">{stats.totalSuperAgents}</span> Super
            </span>
          </div>
        </div>
              ...
              <div className="mt-2 flex items-center">
                <div className="flex-shrink-0">
                  <div className={`h-10 w-10 rounded-full ${
                    stats.mostProductiveAgent.isToolAgent 
      ...
      <div className="px-6 py-3 bg-gray-50 dark:bg-gray-900/50 text-xs text-gray-500 dark:text-gray-400 flex items-center justify-between">
        <span>Statistics based on {tasks.length} tasks and {stats.totalToolAgents + stats.totalSuperAgents} agents</span>
