...
const TeamTasksPage = () => {
  ...
  // Fetch team tasks on component mount and when teamId changes
  useEffect(() => {
    if (teamId) {
      fetchTeamTasks();
    }
  ...
  const handleCreateTask = async (e) => {
    e.preventDefault();
    try {
      await createTeamTask(teamId, formData);
  ...
  const handleUpdateTask = async (e) => {
    e.preventDefault();
    try {
      await updateTeamTask(teamId, currentTask.id, formData);
  ...
  return (
    <Container className="py-4">
      <h1 className="mb-4">Team Tasks</h1>
      ...
      <div className="d-flex justify-content-between mb-4">
        ...
        <Button variant="outline-secondary" onClick={fetchTeamTasks}>
          Refresh
        </Button>
      </div>
      ...
      {loading && tasks.length === 0 ? (
        ...
        <Alert variant="info">No team tasks found.</Alert>
      ) : (
        <Row>
          {tasks.map(task => (
      ...
      <Modal show={showCreateModal} onHide={() => setShowCreateModal(false)}>
        <Modal.Header closeButton>
          <Modal.Title>Create Team Task</Modal.Title>
        </Modal.Header>
      ...
      <Modal show={showEditModal} onHide={() => setShowEditModal(false)}>
        <Modal.Header closeButton>
          <Modal.Title>Edit Team Task</Modal.Title>
        </Modal.Header>
